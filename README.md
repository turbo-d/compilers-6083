...
## Dependencies
- You must have Rust 1.56+ installed. You can find instuctions for installation [here](https://www.rust-lang.org/tools/install).
...
command line program will produce a temporary .ll file containing the LLVM IR of the compiled source file. It will
then run clang to link the .ll file with the runtime static library and create an executable.
...
the `new()` function, and the parse occurs with a call to the `parse()` method. The `parse()` method performs a recursive descent
parse, with a private function corresponding to each non-terminal. The `parse()` method either returns a valid ast, or it
returns a terminal error. This is accomplished using the Rust `Result` type. In either case, the parse may produce an error
log which can be accessed via the `get_errors()` method: 
- If the parse was successful, a valid ast will be produced, but
there may still be errors logged corresponding to warnings and resync errors. The API user must check these errors.
    - If only warnings are logged, the user could continue with full code generation.
    - If errors are logged, these correspond to resync errors where the parser attempted to guess how to continue, and although a valid ast was produced, the semantics of
    the tree may be incorrect and code generation should not be performed.
- In the case of a terminal error during the parse, the parser encountered an error and was unable to proceed, therefore no ast is produced. A call to `get_errors()` will produce the
list of any warnings and resync errors that occured before the terminal error, as well as the description of the terminal error itself.
...
errors like invalid chars or whitespace in an identifier. It also allow us to capture and report a more specific syntax error:
that identifiers can contain underscores, but cannot start with underscores. This was implemented by modifying the grammar for
...
in the source file. If a visitor needs to keep track of declarations with scoping rules as defined in the language spec, a generic symbol table
implementation has been provided in [symtable.rs](./compiler/src/symtable.rs).

The `SymTable` type allows a visitor to associate a string (identifier)
...
methods search for the symbol by first iterating the stack of local scopes, starting with the most recent first, and then finally checking the global scope.
...
type to float type. Similar to the parser, the typechecker can generate a log of errors or warnings as it walks the ast, and again like the parser, you
...
language spec are implemented, as well as an additional function `strcmp()`, used for testing equality of strings. These
functions are implemented in Rust in [/runtime/src/lib.rs](./runtime/src/lib.rs), and we use the extern keyword and
the types provided in the foreign function interface [module](https://doc.rust-lang.org/std/ffi/) of the standard library
...
The only obvious thing that is missing - which I didn't take to be a hard requirement - is column numbers reported in error and warning messages.
