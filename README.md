...
the `new()` function, and the parse occurs with call to the `parse()` method. The `parse()` method performs a recursive descent
...
log which can be accessed via the `get_errors()` method. If the parse was successful, a valid ast will be producted, but
